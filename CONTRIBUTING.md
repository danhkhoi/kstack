...
Contributor mode turns kstack into a self-improving tool. Enable it and Claude Code
will periodically reflect on its kstack experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.kstack/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/kstack/bin/kstack-config set kstack_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork kstack, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use kstack normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.kstack/contributor-logs/`
3. **Fork and clone kstack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where kstack annoyed you)
   ln -sfn /path/to/your/kstack-fork .claude/skills/kstack
   cd .claude/skills/kstack && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using kstack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix kstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ kstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all 15 skills.

## Working on kstack inside the kstack repo

When you're editing kstack skills and want to test them by actually using kstack
...
│   ├── kstack-review -> kstack/review
│   ├── kstack-ship -> kstack/ship
│   └── ...                      <- one symlink per skill (kstack- prefixed)
├── review/
│   └── SKILL.md                 <- edit this, test with /kstack-review
├── ship/
│   └── SKILL.md
├── browse/
...
When E2E tests run, they produce machine-readable artifacts in `~/.kstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.kstack-dev/` for post-mortem debugging and trend analysis.
...
kstack generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/kstack-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/kstack` | `~/.codex/skills/kstack` |
| Hook skills | `hooks:` frontmatter (enforced by Claude) | Inline safety advisory prose (advisory only) |
| `/kstack-codex` skill | Included (Claude wraps codex exec) | Excluded (self-referential) |
...
4. Commit both `{skill}/SKILL.md` and `.agents/skills/kstack-{skill}/SKILL.md`
...
**This is the recommended way to develop kstack.** Symlink your kstack checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/kstack-checkout .claude/skills/kstack
cd .claude/skills/kstack && bun install && bun run build
```

Now every kstack skill invocation in this project uses your working tree. Edit a
template, run `bun run gen:skill-docs`, and the next `/kstack-review` or `/kstack-qa` call picks
it up immediately.

**To go back to the stable global install**, just remove the symlink:

```bash
rm .claude/skills/kstack
```

Claude Code falls back to `~/.claude/skills/kstack/` automatically.
