...
Detailed guides for every kstack skill — philosophy, workflow, and examples.
...
| [`/kstack-upgrade`](#kstack-upgrade) | **Self-Updater** | Upgrade kstack to the latest version. Detects global vs vendored install, syncs both, shows what changed. |
...
Both modes end with a design doc written to `~/.kstack/projects/` — and that doc feeds directly into `/kstack-plan-ceo-review` and `/kstack-plan-eng-review`. The full lifecycle is now: `kstack-office-hours → plan → implement → review → QA → ship → retro`.
...
Visions and decisions are persisted to `~/.kstack/projects/` so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.
...
Eng Review is the only required gate (disable with `kstack-config set skip_eng_review true`). CEO and Design are informational — recommended for product and UI changes respectively.

### Plan-to-QA flow

When `/kstack-plan-eng-review` finishes the test review section, it writes a test plan artifact to `~/.kstack/projects/`. When you later run `/kstack-qa`, it picks up that test plan automatically — your engineering review feeds directly into QA testing with no manual copy-paste.
...
Claude: [Runs full 80-item visual audit on the live site]
        ...
        [Report with before/after screenshots saved to .kstack/design-reports/]
```
...
Claude: [Explores 12 pages, fills 3 forms, tests 2 flows]
        ...
        [Full report with screenshots saved to .kstack/qa-reports/]
```
...
Keep kstack current with one command. It detects your install type (global at `~/.claude/skills/kstack` vs vendored in your project at `.claude/skills/kstack`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.
...
Set `auto_upgrade: true` in `~/.kstack/config.yaml` to skip the prompt entirely — kstack upgrades silently at the start of each session when a new version is available.
...
Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, kstack picks up its comments automatically. No additional configuration.
...
kstack solves this. `/kstack-review` and `/kstack-ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
...
Every false positive you confirm gets saved to `~/.kstack/greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/kstack-retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
