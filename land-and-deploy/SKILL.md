...
```bash
~/.claude/skills/kstack/bin/kstack-review-read 2>/dev/null
```
...
```bash
eval $(~/.claude/skills/kstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
eval $(~/.claude/skills/kstack/bin/kstack-slug 2>/dev/null)
