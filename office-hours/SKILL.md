...
4. **List existing design docs for this project:**
   ```bash
   ls -t ~/.kstack/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.kstack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.kstack/projects/`.
...
```bash
source <(~/.claude/skills/kstack/bin/kstack-slug 2>/dev/null) && mkdir -p ~/.kstack/projects/$SLUG
USER=$(whoami)
...
```bash
PRIOR=$(ls -t ~/.kstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.kstack/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
The design doc at `~/.kstack/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
