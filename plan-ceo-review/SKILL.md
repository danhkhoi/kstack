...
**Design doc check:**
```bash
SLUG=$(~/.claude/skills/kstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.kstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.kstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
HANDOFF=$(ls -t ~/.kstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
mkdir -p ~/.kstack/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```
Write to `~/.kstack/projects/$SLUG/$USER-$BRANCH-ceo-handoff-$DATETIME.md`:
```markdown
...
```bash
source <(~/.claude/skills/kstack/bin/kstack-slug 2>/dev/null) && mkdir -p ~/.kstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.kstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.kstack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.kstack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.kstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```bash
source <(~/.claude/skills/kstack/bin/kstack-slug 2>/dev/null)
rm -f ~/.kstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.kstack/` (user config directory, not project files). The skill preamble
already writes to `~/.kstack/sessions/` and `~/.kstack/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.claude/skills/kstack/bin/kstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.kstack/projects/` only (local, personal reference)
- **C)** Skip
