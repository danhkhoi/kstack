...
const HOST_PATHS: Record<Host, HostPaths> = {
  claude: {
    skillRoot: '~/.claude/skills/kstack',
    localSkillRoot: '.claude/skills/kstack',
    binDir: '~/.claude/skills/kstack/bin',
    browseDir: '~/.claude/skills/kstack/browse/dist',
  },
  codex: {
    skillRoot: '~/.codex/skills/kstack',
    localSkillRoot: '.agents/skills/kstack',
    binDir: '~/.codex/skills/kstack/bin',
    browseDir: '~/.codex/skills/kstack/browse/dist',
  },
};
...
\`\`\`bash
_UPD=$(${ctx.paths.binDir}/kstack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/kstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.kstack/sessions
touch ~/.kstack/sessions/"$PPID"
find ~/.kstack/sessions -mmin +120 -type f -delete 2>/dev/null || true
_CONTRIB=$(${ctx.paths.binDir}/kstack-config get kstack_contributor 2>/dev/null || true)
_PROACTIVE=$(${ctx.paths.binDir}/kstack-config get proactive 2>/dev/null || echo "true")
...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest kstack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows \`UPGRADE_AVAILABLE <old> <new>\`: read \`${ctx.paths.skillRoot}/upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If \`JUST_UPGRADED <from> <to>\`: tell user "Running kstack v{to} (just updated!)" and continue.`;
}
